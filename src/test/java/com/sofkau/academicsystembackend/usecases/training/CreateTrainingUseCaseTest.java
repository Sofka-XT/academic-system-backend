...
class CreateTrainingUseCaseTest {
    ...
    @Test
    @DisplayName("Create training with all the arguments")
    void createTrainingTest(){
        ...
        Mockito.when(repository.save(any())).thenReturn(Mono.just(mapper.mapperToTraining().apply(trainingDTO)));

        var result = createTrainingUseCase.apply(trainingDTO);
